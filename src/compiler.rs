...
pub struct Compiler {
    // Loop vars
    loop_label: Option<Value>, // contains the AST of the current loop/fn
    loop_locals: Option<Vec<Value>>, // contains the params for the binding
}
...
impl Compiler {
    ...
    pub fn eval(&self, ast: Value, env: Rc<Environment>) -> ValueRes {
        Ok(self.eval_to_rc(ast, env)?.to_value())
    }

    fn eval_ast(&self, ast: Value, env: Rc<Environment>) -> Result<Rc<Value>, LispErr> {
        match ast.clone() {
            ...
            Value::Vector(v, _) => {
                let mut evaled = vec![];
                for c in v.iter() {
                    match self.eval(c.clone(), env.clone()) {
                        Ok(l) => evaled.push(l),
                        Err(e) => return Err(core::process_error(&e, &ast)),
                    }
                }
            ...
            Value::HashMap(hm, meta) => {
                ...
                for (k, v) in hm.iter() {
                    let keys = match self.eval(k.clone(), env.clone()) {
                        Ok(v) => v,
                        Err(e) => return Err(core::process_error(&e, &ast)),
                    };
                    let values = match self.eval(v.clone(), env.clone()) {
                        Ok(v) => v,
                        Err(e) => return Err(core::process_error(&e, &ast)),
                    };
    ...
    pub fn eval_to_rc(&self, mut ast: Value, env: Rc<Environment>) -> Result<Rc<Value>, LispErr> {
            ...
            ret = match ast.clone() {
                Value::List(l, _) => {
                    ...
                    match macroexpand(ast.clone(), mut_env.borrow().clone()) {
                        ...
                        (true, Ok(new_ast)) => {
                            return match self
                                .eval_to_rc((*new_ast).clone(), mut_env.borrow().clone())
                            {
                                Ok(v) => Ok(v),
                                Err(e) => {
                                    return Err(core::process_error(&e, &ast));
                                }
                            };
                        }
                    ...
                    match head {
                        ...
                        Value::Symbol(ref headsym) if headsym.name() == "let*" => {
                            ...
                            match local_bindings {
                                Value::Vector(ref binds, _) => {
                                    ...
                                    let local_env = match self.bind_local_env(
                                        binds,
                                        &ast,
                                        mut_env.borrow().clone(),
                                    ) {
                        ...
                        Value::Symbol(ref headsym) if headsym.name() == "do" => {
                            if args.is_empty() {
                                ...
                                for a in args[..l.len() - 1].iter() {
                                    match self.eval_to_rc(a.clone(), mut_env.borrow().clone()) {
                                        Ok(_) => {}
                                        Err(e) => return Err(core::process_error(&e, &ast)),
                                    }
                                }
                        ...
                        Value::Symbol(ref headsym) if headsym.name() == "loop*" => {
                            ...
                            match args[0] {
                                Value::Vector(ref loop_binds, _) => {
                                    ...
                                    // If loop has a body
                                    if args.len() > 1 {
                                        let loop_env = match self.bind_local_env(
                                            loop_binds,
                                            &ast,
                                            mut_env.borrow().clone(),
                                        ) {
                                        ...
                                        println!(
                                            "Saving loop-label {}\n{}",
                                            utils::print_vec(binds_keys.clone()),
                                            loop_body.clone(),
                                        );
                                        self.loop_label = Some(loop_body.clone());
                                        self.loop_locals = Some(binds_keys.clone());

                                        ast = loop_body;
                                        mut_env.replace(loop_env);
                                        continue 'tco;
                        ...
                        Value::Symbol(ref headsym) if headsym.name() == "recur" => {
                            if !args.is_empty() {
                                ...
                                for a in args.iter() {
                                    let eval_res =
                                        match self.eval(a.clone(), mut_env.borrow().clone()) {
                                            Ok(v) => v,
                                            Err(e) => return Err(core::process_error(&e, &ast)),
                                        };
                                ...
                                match (self.loop_label.clone(), self.loop_locals.clone()) {
                                    (Some(loop_body), Some(binds_keys)) => {
                                        // We need to bind the results of recur call
                                        // to initial binding vars
                                        if binds_keys.len() != evaluated_args.len() {
                                            return error_fmt!(
                                                "recur received wrong number of params: expected {}",
                                                binds_keys.len()
                                            );
                                            ...
                                            match k {
                                                Value::Symbol(sym) => {
                                                    let var = Var::new(
                                                        ...
                                                        (*sym).clone(),
                                                        Rc::new(evaluated_args[i].clone()),
                                                    );
                                                    // Insert the let bindings into the local env, as Vars
                                                    mut_env.borrow().insert(
                                                        (*sym).clone(),
                                                        Value::Var(var).to_rc_value(),
                                                    );
                                                }
                                                _ => {
                                                    return error_fmt!("Invalid use of binding");
                                                }
                                            }
                                        }
                                    ...
                                    _ => {
                                        error_fmt!(
                                            "recur called outside a function or a loop: {}",
                                            ast
                                        )
                                    }
                                }
                        ...
                        Value::Symbol(ref headsym) if headsym.name() == "if" => {
                            ...
                            let cond = match self.eval(args[0].clone(), mut_env.borrow().clone()) {
                                Ok(v) => v,
                                Err(e) => return Err(core::process_error(&e, &ast)),
                            };
                            match cond {
                                ...
                                Value::List(_, _) => {
                                    ...
                                    for p in params_v.clone() {
                                        match &*p {
                                            ...
                                            _ => {
                                                return error!(
                                                    "Function arguments must be a vector."
                                                )
                                            }
                                        }
                                    }
                        ...
                        Value::Symbol(ref headsym) if headsym.name() == "eval" => {
                            if args.len() != 1 {
                                return error!(
                                    "Wrong number of arguments given to eval. Expecting 1"
                                );
                            }
                            // NB eval expects a form, not a string.
                            // So first evaluate the argument, and then evaluate the form in the top environment
                            ast = match self.eval(args[0].clone(), mut_env.borrow().clone()) {
                                Ok(v) => v,
                                Err(e) => return Err(core::process_error(&e, &ast)),
                            };
                            mut_env
                                .replace(Rc::new(mut_env.borrow().get_main_environment().clone()));
                            continue 'tco;
                        }
                        Value::Symbol(ref headsym) if headsym.name() == "quote" => {
                            if args.len() != 1 {
                                return error!(
                                    "Wrong number of arguments given to quote. Expecting 1"
                                );
                            }
                        ...
                        Value::Symbol(ref headsym) if headsym.name() == "defmacro" => {
                            ...
                            let r = match self.eval(macro_args, mut_env.borrow().clone()) {
                                Ok(v) => v,
                                Err(e) => return Err(core::process_error(&e, &ast)),
                            };
                            match r {
                        ...
                        Value::Symbol(ref headsym) if headsym.name() == "try" => {
                            ...
                            match self.eval(args[0].clone(), mut_env.borrow().clone()) {
                                ...
                                Err(ref e) if args.len() >= 2 => {
                                    ...
                                    match args[1].clone() {
                                        Value::List(c, _) => {
                                            let catch_env = mut_env.borrow().bind(
                                                list![c[1].clone()].to_rc_value(),
                                                vec![ex],
                                            )?;
                                            Ok(self.eval_to_rc(c[2].clone(), catch_env)?)
                                        }
                        ...
                        Value::Symbol(ref headsym) if headsym.name() == "var" => {
                            if args.len() != 1 {
                                return error!(
                                    "Wrong number of arguments passed to var. Expecting 1"
                                );
                            }
                            match args[0].clone() {
                                Value::Symbol(sym) => {
                        ...
                        _ => {
                            ...
                            let ref f =
                                match self.eval_to_rc(head.clone(), mut_env.borrow().clone()) {
                                    Ok(x) => x,
                                    Err(e) => return Err(core::process_error(&e, &ast)),
                                };
                            ...
                            match &*f.borrow() {
                                ...
                                Value::Lambda {
                                    ...
                                    for arg in args.iter() {
                                        match self.eval(arg.clone(), mut_env.borrow().clone()) {
                                    ...
                                    if let Some((a, p)) = core::find_ast_and_params_by_arity(
                                        lambda_ast, params, args_count,
                                    ) {
                ...
                _ => self.eval_ast(ast, mut_env.borrow().clone()),
            };
    ...
    fn bind_local_env(
        ...
        for (b, e) in binds.iter().tuples() {
            match b {
                Value::Symbol(sym) => {
                    let val = match self.eval_to_rc(e.clone(), local_env.clone()) {
                        Ok(v) => v,
                        Err(e) => return Err(core::process_error(&e, ast)),
                    };
                    let var = Var::new(
